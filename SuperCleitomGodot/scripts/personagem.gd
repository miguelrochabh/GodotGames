...
func _fixed_process(delta):
	...
	var walk_left = (Input.is_action_pressed("move_left") or left) and vivo and not fim
	var walk_right = (Input.is_action_pressed("move_right") or right or fim) and vivo
	var jump = (Input.is_action_pressed("jump") or up) and vivo and not fim
	...
	if not rolled:
		if (on_air_time < JUMP_MAX_AIRBORNE_TIME and jump and not prev_jump_pressed and not jumping):
			pular()
	if rolled:
		if Input.is_action_pressed("jump") or up:
			if fim: return
			if not fim:
				pular()
